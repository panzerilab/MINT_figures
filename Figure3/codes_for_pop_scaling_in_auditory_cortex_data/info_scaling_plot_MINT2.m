function info_scaling_plot_MINT2()
%%
% Plot the results
...
load('Results\current_analysis\info_analysis_with_population_sizefixedextr.mat', ...
...
avgSI_pop_partitions_pop_size_ind = avgSI_popnaive_partitions_pop_size_ind;
avgCI_pop_partitions_pop_size_ind = avgCI_popnaive_partitions_pop_size_ind;
avgII_pop_partitions_pop_size_ind = avgII_popnaive_partitions_pop_size_ind;

IIpersubject=squeeze(sum(avgII_pop_partitions_pop_size_ind, 3));
IIpersubject(:,1)=IIpersubject(:,1)/20;
IIpersubject(:,2)=IIpersubject(:,2)/100;
IIpersubject(:,3)=IIpersubject(:,3)/100;
IIpersubject(:,4)=IIpersubject(:,4)/100;
...
[avgII_pop_partitions_pop_size_ind_across_subj_pairs] =[nonzeros(IIpersubject(:,1)) nonzeros(IIpersubject(:,2)) nonzeros(IIpersubject(:,3)) nonzeros(IIpersubject(:,4))] ;%pool_infodata_to_plot_as_info_vs_popsize(avgII_pop_partitions_pop_size_ind,Nsessions,sample_combinations_pop_size);
...
figure('Visible','on'); set(gcf,'Position',[00, 00, 600, 300]); hold on;
subplot(1,2,1)
...
h = legend([fitSI,fitCI,fitII,behav_perf],{'I(S,$\hat{S}$)','I(C,$\hat{C}$)','II(S,[$\hat{S},\hat{C}$],C)','I(S,C)'},...
    'Location','southeast','Interpreter', 'latex'); 
ylabel('bits');xlabel('population size'); %xlim([2 18]); ylim([0 0.5])

subplot(1,2,2)
avgSI_pop_partitions_pop_size_ind = avgSI_popqe_partitions_pop_size_ind;
avgCI_pop_partitions_pop_size_ind = avgCI_popqe_partitions_pop_size_ind;
avgII_pop_partitions_pop_size_ind = avgII_popqe_partitions_pop_size_ind;
...
filename = ['Figures/Population_based_SI_CI_II_fixedextr',char(fit_function),'.pdf']; 
